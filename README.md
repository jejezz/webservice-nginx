...
### 빈 장비에서는 관리자 콘솔로 들어갑니다

로그인 화면의 **설정 버튼(⚙)** → `zoomon` 과 `bootstrap.sh` 에서 정한 비밀번호 →
헤더의 **구축 마법사**.

일반 로그인은 아직 되지 않습니다. 그 계정은 MariaDB 의 `administrator` 테이블에
있는데, **그 테이블을 만드는 것이 마법사의 2단계**이기 때문입니다. 관리자 콘솔은
`config.json` 만으로 인증되어 그 고리 밖에 있습니다.

DB 를 세우고 자기 계정을 만든 뒤부터는 일반 로그인으로 들어가면 됩니다.
