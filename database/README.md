...
이 서버에는 **MariaDB 10.6.23이 이미 설치되어 실행 중**입니다. (`127.0.0.1:3306`)
`kamailio` 데이터베이스가 사용 중이므로, 아래 스크립트는 **기존 데이터를 건드리지 않도록** 만들어져 있습니다.
...
`bind_address`는 기본이 `127.0.0.1`(로컬 전용)입니다. 외부 접속을 허용하려면 `0.0.0.0`으로 바꾸되
방화벽으로 접근 범위를 반드시 제한하세요.
...
`host = %` 이므로 **어느 주소에서든 접속할 수 있습니다.** 실제로 외부에서 닿는지는 두 가지에 달려 있습니다.

1. `[server]`의 `bind_address` — `127.0.0.1`이면 로컬만, `0.0.0.0`이면 모든 인터페이스
2. 라우터에서 3306을 포워딩했는지 여부

> 계정 권한을 넓게 열어둔 만큼 노출 범위는 좁게 유지하는 편이 좋습니다.
> 3306을 인터넷에 포워딩하지 않는 한 접근 가능 범위는 LAN과 컨테이너·VM으로 한정됩니다.
