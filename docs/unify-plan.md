...
**이건 fail-closed 와 별개의 결정이라 여기서 고치지 않습니다.** 길이 여럿이고
(마법사 앞 단계만 인증 밖으로 · `superAdmin` 으로 세션을 낼 수 있게 · 파일
저장소로 첫 계정을 심고 DB 가 서면 옮기기), 어느 쪽이든 인증 경계를 옮기는
일입니다. **Phase 1 의 마지막 항목으로 따로 세웁니다.**
...
- [ ] **빈 장비에서 마법사에 들어갈 길** — 지금은 DB 가 서야 로그인이 되고,
      DB 를 세우는 것이 마법사 2단계다. 인증 경계를 어디로 옮길지 정해야 한다 (§5-②)
