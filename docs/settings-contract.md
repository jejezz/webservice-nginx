...
| `settings.ini` | 화면 · 편집기 (뼈대는 `--init`) | **사람이 정한 값** |
...
`settings.ini` 는 커밋하지 않으므로 **새 장비에는 없습니다.** 화면(서비스
대시보드·구축 마법사)에서 저장하면 그때 만들어집니다. 화면을 띄울 수 없는
장비를 위해, 스키마에서 주석 달린 뼈대를 만드는 길을 함께 둡니다.
...
항목마다 `label` · `help` · `patternHint` · `effect` 가 주석으로 들어가고,
`default` 가 있는 항목만 값이 채워집니다. **값이 이미 있는 파일은 덮지
않습니다** — 사람이 정한 값은 되돌릴 수 없기 때문입니다.

뼈대는 어디까지나 편의입니다. 검증은 저장할 때 화면이 하고, `--apply` 가 root
로 한 번 더 합니다 — 손으로 적은 파일도 같은 관문을 지납니다.
