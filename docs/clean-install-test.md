...
**여기서 일반 로그인을 시도하지 마세요 — 아직 됩니다가 아닙니다.**

로그인 화면의 **설정 버튼(⚙)** 을 누르고 `zoomon` 과 `bootstrap.sh` 에서 정한
비밀번호로 관리자 콘솔에 들어간 뒤, 헤더의 **구축 마법사** 를 누릅니다.
기본 비밀번호는 없습니다.

관리자 계정은 MariaDB 의 `administrator` 테이블에 있고 **그 테이블은 2단계에서
생깁니다.** 그래서 빈 장비에서 일반 로그인은 `503` 입니다. 콘솔은 `config.json`
만으로 인증되므로 그 고리 밖에 있습니다.

> 콘솔 세션의 유효 시간은 **30분**입니다 (일반 세션은 120분). 마법사 중간에
> 끊기면 같은 길로 다시 들어오면 됩니다 — 진행 상황은 파일에 있습니다.
