...
# 판정에만 넣고 **사람 화면에는 찍지 않는다.**
#
#     judge ok "광고 중인 공인 IP 가 현재 값과 같습니다"
#
# 사람용 문구를 이미 따로 갖고 있는 스크립트에서 쓴다 (check-public-ip.sh 의
# "판정: …" 처럼). 같은 말을 두 번 하지 않기 위해서다.
judge() { _check_add "$1" "${*:2}"; }
