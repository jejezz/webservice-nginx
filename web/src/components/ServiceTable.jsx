...
function DetailRow({ service }) {
  ...
  const items = [
    ...
    ['등록 위치', (service.sources || []).map((s) => (s === 'nginx' ? 'nginx.ini' : 'ecosystem.config.js')).join(' + ') || '—'],
            ...
            >
              ...
              <TableCell>
                <div className="flex items-center gap-2.5">
                  ...
                  {!service.location && (
                    <Badge variant="secondary" className="hidden sm:inline-flex" title="nginx.ini 라우트 없이 포트로 직접 접근">
                      직결
                    </Badge>
                  )}
                </div>
