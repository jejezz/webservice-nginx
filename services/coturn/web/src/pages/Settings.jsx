 ...
 *    sudo 를 부르는 길은 두지 않습니다 — coturn 을 재기동하는 일을 웹 화면의
 *    단추 하나에 걸어 두지 않으려는 것입니다 (services/janus/web 의 같은
 *    화면과 동일한 자세).
 *
 * static_auth_secret 은 여기 없습니다 — settings-schema.json 자체에 그
 * 필드가 없습니다. 사람이 입력하는 값이 아니라 install.sh --apply 가
 * 생성하는 비밀이기 때문입니다 (server/src/config.js 의 주석 참고).
 *
      ...
      */}
      <Card className={pending ? 'border-warning' : undefined}>
        <CardHeader className="pb-3">
          ...
          <p className="text-xs text-muted-foreground">
            이 화면은 파일에 값을 적을 뿐입니다. coturn 에 반영하는 것은
            <strong> 사람이 터미널에서</strong> 합니다 — 패키지 설치와 재기동이 걸리는 일이라
            웹 단추에 걸어 두지 않았습니다.
          </p>
        </CardHeader>
          ...
          {state.values.public_ip ? (
            <Alert>
              ...
              <AlertDescription className="space-y-1 text-xs">
                <p>
                  <span className="font-mono">UDP+TCP {state.values.listening_port}</span> 과{' '}
                  <span className="font-mono">UDP {state.values.relay_port_range}</span> 를
                  이 장비로 포워딩하세요.
                </p>
                <p>
                  Janus 의 WebRTC 미디어 포워딩과는 <strong>다른 범위</strong>입니다 — 추가로 열어야 합니다.
                </p>
                <p>
                  공인 IP 는 회선 따라 바뀝니다. Janus 쪽 값을 그대로 물려받고 있다면
                  <span className="font-mono"> services/janus/check-public-ip.sh</span> 가 갱신해 둔 값을
                  자동으로 따라갑니다.
                </p>
              </AlertDescription>
            </Alert>
            ...
            <Alert variant="warning">
              ...
              <AlertDescription className="text-xs">
                공인 IP 가 비어 있어 <strong>LAN 전용</strong>으로 설치됩니다 — 이 TURN 서버를 만든
                이유(셀룰러 모바일의 NAT 통과)가 무효화됩니다. 공인 IP 를 넣거나,
                services/janus/settings.ini 쪽에 넣어 물려받게 하세요.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
