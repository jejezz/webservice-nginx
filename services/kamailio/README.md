...
**4-1-1. 점검 출력을 읽는 법.** 아직 적용하지 않았으면 "설치본이 저장소와 같은가"
절은 **다르다고 나오는 것이 정상입니다** — `/etc/kamailio/kamailio.cfg` 가 아직
배포판 파일이기 때문입니다. 줄이 어느 쪽 것인지 출력이 직접 말해 줍니다.

```
(저장소에는 있고 설치본에는 없는 줄 171개 — 앞 4개)
< # │  이 파일은 배포판 설정의 **포크** 입니다.
(설치본에만 있는 줄 39개 — 비밀이 섞일 수 있어 내용은 찍지 않는다)
```

설치본 쪽 줄을 찍지 않는 것은 거기에 비밀번호가 들어 있기 때문입니다
(`DBURL`). 그 출력은 화면과 마법사의 JSON 을 타고 나갑니다.

**⚠️ `[!!] DB 로그인 실패` 가 있으면 먼저 그것부터입니다.** 이 상태로 `--apply`
를 해도 4-3-1 에서 막힙니다 — 아무것도 바꾸지 않고 멈추므로 해롭지는 않습니다.
원인과 해결은 [T-2](#t-2-db-오류로-기동하지-않음--access-denied-for-user-kamailio)
입니다.
