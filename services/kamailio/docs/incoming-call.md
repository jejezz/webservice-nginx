...
않습니다. **이 트랜잭션에는 나가는 브랜치가 없습니다** — `lookup()` 이 실패해
아무 데도 relay 하지 않은 채 `t_newtran()` + 로컬 `t_reply("183")` 만 했기
때문입니다. 브랜치가 없으므로 `fr_inv_timer` 가 아니라 더 짧은 타이머가 적용되는
것으로 보입니다.
...
트랜잭션 수명을 명시적으로 늘려야 합니다. `ts_store()` 앞뒤에서 `t_set_fr()` 로
이 트랜잭션에만 긴 타이머를 주는 것이 후보입니다.

```
t_set_fr(0, 90000);   # 이 트랜잭션의 fr_timer 만 90초로
ts_store();
```

**아직 적용하지 않았습니다.** `kamailio.cfg` 를 고치고 `sudo ./install.sh --apply`
가 필요하며, 어느 타이머가 실제로 이 경로를 지배하는지 값을 바꿔 가며 재 봐야
확정됩니다. 위 표가 그 측정의 출발점입니다.
