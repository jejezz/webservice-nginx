...
| 확인 입력이 다름 | `400 password_mismatch` | 확인 칸만 비우고 다시 |
| 확인 입력이 같음 | `403 pending_approval` | "승인 요청이 등록되었습니다" 안내 |
| 승인 대기 중 + 같은 비밀번호 | `403 pending_approval` | "승인 대기 중입니다" 안내 |
| 승인 대기 중 + 다른 비밀번호 | `403 password_confirm_required` (`reason: reset`) | **비밀번호 확인 칸이 열림** |
| 승인된 계정 + 비밀번호 불일치 | `401 invalid_credentials` | 오류 |
| 이메일 형식 아님 | `400 invalid_email` | 오류 |
| DB 접속 불가 | `503` | 오류 |

승인 대기는 실패로 세지 않으므로 로그인 시도 제한(IP당 5회)에 걸리지 않습니다.
확인을 요구하는 두 응답도 마찬가지입니다 — 자격 증명을 틀린 것이 아닙니다.

### 비밀번호가 새로 저장될 때는 확인을 받습니다

**신규 등록**과 **승인 전 재설정**, 두 자리에서만 확인 칸이 열립니다. 둘 다
`password_hash` 를 새로 쓰는 자리이고, **확인 없이는 쓰지 않습니다.**

없으면 어떻게 되는지가 이유입니다. 처음 치는 비밀번호에 오타가 나면 그것이
그대로 계정 비밀번호가 되는데, **오타는 승인이 난 뒤에야 드러납니다.** 그때는
로그인이 안 되는 이유를 알 길이 없습니다 — 화면은 그냥 "비밀번호가 올바르지
않습니다" 라고만 합니다.

승인 전에 비밀번호를 다시 설정할 수 있게 둔 것은 그대로입니다. 아직 아무 권한이
없으므로 위험하지 않고, 오타를 승인 전에 바로잡을 수 있습니다.

> 확인 칸이 열린 뒤 **이메일을 바꾸면 단계가 처음으로 돌아갑니다.** 다른 계정
> 얘기가 되기 때문입니다.
>
> 이미 로그인한 사람에게 다시 묻는 자리(`POST /verify-password`)는 이 흐름을
> 타지 않습니다. 그쪽은 `verifyOnly` 로 **아무것도 만들거나 고치지 않고 확인만**
> 합니다 — 세션은 있는데 계정이 사라진 상태로 물으면 엉뚱한 승인 요청이 생깁니다.
