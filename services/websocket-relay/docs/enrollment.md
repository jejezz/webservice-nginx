...
```json
{ "method": "device.permissions", "deviceId": 36, "canCall": false }
```

```json
{ "method": "device.permissions", "deviceId": 36, "ok": true,
  "canCall": false, "canControl": true, "devices": [ ... ] }
```

> ⚠️ **`enroll.approve` 와 규칙이 반대입니다.** 승인은 값을 안 보내면 **꺼짐**
> 이지만(실수로 권한이 켜지지 않게), 여기서는 **안 보내면 건드리지 않습니다** —
> "통화만 끄겠다" 는 요청이 제어까지 함께 꺼 버리면 안 되기 때문입니다.
> 둘 다 안 보내면 `nothing_to_change` 로 돌려줍니다. 조용히 성공하면 부르는
> 쪽은 바뀐 줄 압니다.

바뀐 권한은 그 단말이 다음에 `/register/mobile` 을 부를 때 응답의 `canCall` ·
`canControl` 로 따라잡습니다. 즉시 알려 주는 푸시는 없습니다.
...
- **통화 / 제어를 따로** 고를 수 있게. 한쪽만 주는 경우가 실제로 있습니다
  (예: 부모님 폰은 전화만). 승인 뒤에 바꾸는 것은 `device.permissions` 입니다
- **남은 시간** — `expires_at`
- **거절**
