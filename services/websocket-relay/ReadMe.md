...
| 모바일 단말 | **승인 대기 처리**, 동/호 필터, 통화·제어 권한 토글, **시험 푸시**, 추가·수정·삭제. 단지 ID 불일치·SIP 내선 없음·푸시 실패를 눈에 보이게 합니다 |
...
키가 통하는 것과 **그 폰에 알림이 뜨는 것**은 다른 문제입니다. 토큰이 다른
프로젝트 것이거나, 앱을 지웠다 다시 깔았거나, 앱이 알림 채널을 안 만들었을 수
있고 셋 다 서버 로그에는 "0 messages were sent successfully" 로만 남습니다.

'모바일 단말' 표의 각 행에 종 모양 버튼이 있습니다. 두 가지로 보냅니다.

| | 단말에 뜨나 | 기록을 바꾸나 | 무엇에 답하나 |
|---|---|---|---|
| 토큰 검증만 | 아니오 | 아니오 | 그 토큰이 이 프로젝트에서 아직 유효한가 |
| 실제 발송 | 예 | 예 (다른 발송과 같은 규칙) | 앱이 실제로 알림을 띄우는가 |

실패하면 코드와 **고칠 방법**을 함께 보여 줍니다 (`libs/push.ts` 의 `HINTS`).
초인종과 다른 `method: test-push` 로 보내므로 앱이 통화 화면을 띄우지 않습니다.

서버에 들어갈 수 있으면 같은 일을 CLI 로도 합니다 — 단말이 아직 없을 때는
이쪽뿐입니다:

```bash
node tools/fcm.js selftest          # 단말 없이 발송 경로 전체
