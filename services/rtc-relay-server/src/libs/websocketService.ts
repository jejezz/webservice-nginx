 ...
 * 
 * - Dual protocol support: RTC (/ws) and IoT (/iot) WebSocket endpoints
...
const WS_PATHS = {
    /** @brief Standard RTC WebSocket path */
    RTC: '/ws',
    /** @brief Alternative RTC WebSocket path with trailing slash */
    RTC_ALT: '/ws/',
    /** @brief IoT device WebSocket path */
    IOT: '/iot'
} as const;
...
export class WebSocketService {
     ...
     * @param callFusion The CallFusion application instance
     ...
     */
    public static createInstance(callFusion: CallFusion): WebSocketService {
     ...
     * - URL-based routing for RTC (/ws) and IoT (/iot) protocols
     ...
     */
    private constructor(callFusion : CallFusion) {
        this.app = callFusion;
        this.express = callFusion.expressApp;
        
        let opt = { 
            server: callFusion.httpsServer, 
            //path: '/ws/iot', 
            clientTracking: false 
        };

        this.rtcServer = new WebSocketServer({ server: callFusion.httpsServer, clientTracking: false });
        /**
         ...
         * - /ws or /ws/ → RTC message handling for voice/video calls
         * - /iot → IoT message handling for device communication
    ...
    private _handleRtcBroadcastMessage(ws: WebSocket, msg : ClientMessage): void {
        ...
        if(!this.app.roomTable.isJoinedToRoom(parseInt(msg.roomid), ws)) {
            logger.warn("client is not joined to the room");
            return;
        }

        this.app.roomTable.send(parseInt(msg.roomid), 
            parseInt(msg.clientid), msg ?? '');
    }
    ...
    private _handleRtcRemoveClient(ws:WebSocket, msg: ClientMessage) : boolean {
        ...
        this.app.roomTable.removeClientFromRoom(parseInt(msg.roomid), parseInt(msg.clientid), ws);
        return true;
    }
    ...
    private async _handleIotStatusAsync(ws: WebSocket, msg: IoTMessage) {
        ...
        // }
        await this._handleIotSendStatusWithDelayAsync(room, msg, 500);
     ...
     */
    private async _handleIotSendStatusWithDelayAsync(room: RtcRoom, msg: IoTMessage, delayMs: number = 200) {
        for (const client of room.clients) {
            if (!client[1].initiator && client[1].isSubscribed()) {
                msg.clientid = Number(client[1].cid);
                client[1].send(msg);
 ...
 * @param {CallFusion} callFusion - The CallFusion instance.
