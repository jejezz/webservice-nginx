...
| 10-3 | `/sip/`(Kamailio WS) 라우트를 유지할지 | ✅ **유지** — 아래 |
| 10-4 | 안드로이드 앱의 경로 | 🔸 **결정 대기** — 아래에 근거를 정리했습니다 |
...
쓰는 데가 없으니 지울 근거가 있어 보이지만, **유지하는 편이 맞습니다.**

- 지우는 이득이 거의 없습니다. 리스너는 루프백 전용이고 nginx 라우트 한 줄입니다.
- 되살리는 비용은 큽니다. `services/kamailio/docs/websocket-plan.md` 가 1단계까지
  끝내 둔 자산이고, 지우면 그 문서가 실물과 어긋나기 시작합니다.
- **모바일이 셀룰러에서 붙는 길**로 다시 필요해질 가능성이 높습니다 (10-4 참고).

다만 지금은 아무도 안 쓰므로, 대시보드가 이 서비스를 "정상" 으로 표시한다고 해서
누군가 쓰고 있다는 뜻은 아니라는 점을 알아 두어야 합니다.
...
**결정은 미뤄 둡니다.** 지금 당장 막히는 것이 없고, TLS 작업의 결과를 보고
정하는 편이 정보가 더 많습니다.
