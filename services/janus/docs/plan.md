...
부수 효과가 하나 더 있는데, 좋은 쪽입니다. Kamailio 가 보는 출발지 IP 가
`192.168.0.252` 이고 Janus 가 Contact 에 적는 주소도 같으므로 `route[NATDETECT]`
가 NAT 를 잡지 않습니다. `FLT_NATS` 가 서지 않으면 `route[NATMANAGE]` 가 첫 줄에서
그냥 돌아가므로 **rtpengine/rtpproxy 를 부르지 않습니다.** 지금 `WITH_NAT` 이
켜져 있고 rtpengine 데몬이 없는 상태와 정확히 맞아떨어집니다.

> 이 추론은 반드시 실물로 확인해야 합니다. 4단계 검증 항목에 넣어 두었습니다.
...
| 5-3 | 미디어가 실제로 흐르는지 | 대시보드 Media 화면의 비트레이트 · `chrome://webrtc-internals` |
