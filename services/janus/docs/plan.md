...
#### ✅ 그때가 왔습니다 — 둘 다 엽니다 (2026-08-22)

WebRTC 클라이언트가 붙을 **WebSocket 주소가 필요해졌습니다.** 그래서 확장을
했는데, `health_port` 가 아니라 **라우트 쪽에 `port`** 를 붙였습니다.

```ini
[service]
ports = 8088                  ; 헬스는 여기로 (바뀐 것 없음)

[route:ws]
location  = /janus-ws
port      = 8188              ; 이 라우트만 WS 포트로
websocket = true
```

방향이 반대일 뿐 같은 문제를 풉니다. 헬스 규약은 **한 글자도 바뀌지 않고**,
`/janus-api/info` 가 그대로 헬스입니다. WS 는 옆문으로만 나갑니다.

| | REST (8088) | WebSocket (8188) |
|---|---|---|
| 밖에서 | `https://<서버>/janus-api` | `wss://<서버>/janus-ws` |
| 쓰는 쪽 | 대시보드의 시험 통화 · **헬스** | WebRTC 클라이언트 |

WS 트랜스포트는 `ws_ip = "127.0.0.1"` 로 **루프백에만** 답니다. TLS 는 nginx 가
끊으므로 Janus 쪽 `wss` 는 계속 꺼 둡니다 — 인증서를 두 벌 두지 않습니다.
...
사실로 확인**됐습니다. `28443 → 443` 이 그대로 동작합니다.

> 위는 그때의 기록입니다. 2026-08-29 에 주소가 바뀌었습니다 — 그 DDNS 이름은
> 삭제됐고 `28443` 도 닫혔습니다. 지금은 `https://c-a3f19c04.rtc.zoomon.art`
> (표준 443)로 붙습니다. 결론(추가 포워딩 불필요)은 그대로입니다. 9단계에 남는 것은
...
| | 할 일 | 상태 |
|---|---|---|
| 9-1 | `nat_1_1_mapping` — 대시보드 '설정' 화면 | ✅ 준비됨 — **적용은 sudo 필요** |
...
대시보드의 **설정** 화면(`/janus/dashboard/settings`)에서 값을 넣고 저장한 뒤,
화면이 알려 주는 명령을 터미널에서 실행합니다.
...
그다음 공유기에서 **UDP `<설정한 범위>` → `192.168.0.252`** 를 엽니다.

#### 9-1 — 값은 `settings.ini` 에, 적용은 사람이

`janus.jcfg` 에 IP 를 직접 적으면 회선이 바뀔 때마다 커밋이 생기고, 다른 장비에
그대로 설치하면 남의 주소를 광고합니다. 그래서 값만 따로 받습니다.

```
대시보드 '설정' 화면  ──쓴다──▶  settings.ini  ──읽는다──▶  install.sh --apply
                                                              │
                            사람이 터미널에서 sudo 로 실행 ◀───┘
```

| `settings.ini` 의 `public_ip` | `install.sh --apply` 가 하는 일 |
|---|---|
| 값 있음 | `nat_1_1_mapping` 을 그 값으로 치환 |
| **비어 있음** | `nat_1_1_mapping` 과 `keep_private_host` **두 줄을 지운다** (LAN 전용) |

"비면 지운다" 를 기본으로 둔 이유는, 자리표시자가 남으면 Janus 가 그 문자열을
주소로 알아듣고 **외부 통화가 조용히 무음이 되기** 때문입니다.

`rtp_port_range` 는 자리표시자를 두지 않고 설치본의 값을 덮어씁니다 —
`janus.jcfg` 가 그 자체로 온전한 설정으로 남아 있게 하려는 것입니다 (그대로
복사해도 동작합니다).

#### 대시보드는 sudo 를 부르지 않습니다

이 화면은 이 대시보드에서 **유일하게 쓰기가 있는 자리**입니다. 그래도 하는 일은
`settings.ini` 에 값을 적는 것뿐이고, 명령은 **보여 주기만** 합니다. Janus 를
재기동하는 일을 웹 단추 하나에 걸어 두지 않으려는 것입니다.

`settings.ini` 는 root 로 도는 스크립트가 읽는 입력이므로 **검증을 두 곳에서**
합니다 — 저장할 때(대시보드)와 설치할 때(`install.sh`). 대시보드를 우회해 파일을
손으로 고쳐도 `install.sh` 가 같은 관문에서 막습니다. 형식이 어긋나면 아무것도
바꾸지 않고 멈춥니다.

겹침 검사도 설정값을 봅니다. 템플릿에 남아 있는 옛 값을 보면 실제로는 겹치는데
검사가 통과하는 일이 생기기 때문입니다.

```
[!!]   rtpproxy(10200-19999) 와 Janus WebRTC(15000-15200) 가 겹칩니다
```
...
1. `server/src/settings.js` 의 `SCHEMA` 에 항목을 더한다 (라벨·설명·검증·효과)
2. `install.sh` 에서 `settings_get <키>` 로 읽어 원하는 곳에 반영한다

화면은 서버가 준 스키마로 그려지므로 새 항목이 저절로 나타납니다.
...
> 2026-08-29 에 바뀌었습니다. 예전에는 공유기의 DDNS(`jejezzhome.iptime.org`)를
> 봤지만 그 이름은 삭제됐습니다. **공유기가 알아서 갱신하던 것이 없어졌으므로,
> 회선 IP 가 바뀌면 등록기관의 A 레코드를 사람이 고쳐야 합니다.**
...
브라우저**로 `https://c-a3f19c04.rtc.zoomon.art/janus/dashboard/test-call` 에
...
| | 할 일 | 상태 |
|---|---|---|
| 10-1 | `README.md` 를 실제 결과로 갱신 | ✅ |
| 10-2 | 빈 장비에서 재현되는지 | ✅ `bootstrap.sh` |
| 10-3 | `/sip/`(Kamailio WS) 라우트를 유지할지 | ✅ **껐습니다** — 파일은 남김 (아래) |
| 10-4 | 안드로이드 앱의 경로 | ✅ **Janus** (2026-08-21) |

#### 10-2 — 무엇이 재현을 막고 있었나

`bootstrap.sh` 를 만들면서 드러난 것은, **Janus 를 소스에서 빌드했다는 사실이
어디에도 적혀 있지 않았다**는 점입니다. 패키지가 아니라 `/opt/janus` 에 직접
빌드해 넣은 것이라, 새 장비에서는 그 단계부터 막힙니다.

의존성 목록은 짐작이 아니라 **실측**입니다 — 설치된 바이너리와 `.so` 가 무엇에
링크돼 있는지 `ldd` 로 확인해서 뽑았습니다.

```
janus                 libglib-2.0 · libjansson · libnice · libsrtp2 · libssl · libconfig · libcurl
libjanus_sip.so       libsofia-sip-ua      ← 이게 없으면 SIP 플러그인이 안 만들어진다
libjanus_http.so      libmicrohttpd        ← 이게 없으면 /janus-api 가 없다
```

빌드 자체는 스크립트가 하지 않습니다. 오래 걸리고 실패 지점이 많아 사람이
보면서 해야 하고, 특히 **`configure` 요약에서 SIP plugin 과 REST transport 가
`yes` 인지** 확인해야 합니다 — 거기서 `no` 로 지나가면 나중에 "플러그인이 없다"
로만 나타나 원인을 짚기 어렵습니다.
...
| 확인 | 결과 |
|---|---|
| nginx 라우트 | ✅ 살아 있음 (`/sip/` → `127.0.0.1:5080`, WS 업그레이드 아니면 400) |
| Kamailio WS 리스너 | ✅ `tcp:127.0.0.1:5080` |
| 현재 연결 | 0 — **쓰는 단말이 없습니다** |
| 지금 안드로이드 앱 | WS 가 아니라 **평문 SIP/UDP 5060** 으로 붙습니다 (`192.168.0.67:5160`) |

처음에는 **유지**로 결론냈습니다. 근거는 "모바일이 셀룰러에서 붙는 길로 다시
필요해질 가능성이 높다" 였습니다.

##### ⚠️ 그 전제가 무너졌습니다 (2026-08-21)

10-4 에서 **모바일도 Janus 로 붙기로 정했습니다.** 그러면 `/sip/` 를 쓸 사람이
아무도 없습니다.
...
`/sip/` 를 남겨 두는 값은 이제 "나중에 쓸지도 모른다" 하나뿐인데, 그 "나중" 이
가리키던 것이 방금 ①로 정해졌습니다.

**✅ 껐습니다 (2026-08-21).** 지우지 않고 `enabled = false` 로 내렸습니다.

- `nginx-conf/service.ini` 한 줄이면 되고 되돌리기도 한 줄입니다.
- `websocket-plan.md` 는 1단계까지 실제로 해 본 기록이라 남길 값이 있습니다.
  파일을 지우면 그 문서가 실물 없는 계획서가 됩니다.
- Kamailio 의 WS 리스너(`tcp:127.0.0.1:5080`)는 루프백 전용이라 그대로 둬도
  바깥에 노출되지 않습니다.

```
$ ./nginx/install_nginx_stack.sh --check
  skip    kamailio           (enabled = false)
  ...
6 services, no conflicts.
```

⚠️ **`enabled = false` 인 서비스는 생성기가 스캔 단계에서 건너뜁니다** — 포트·경로
충돌 검사도 받지 않습니다. 다시 켤 때 그 검사를 처음 받는 셈이므로, 그때
`--check` 를 꼭 먼저 돌려야 합니다.

Kamailio 의 WS 리스너(`tcp:127.0.0.1:5080`)는 그대로 둡니다. 루프백 전용이라
바깥에 노출되지 않고, `kamailio-websocket.cfg` 를 걷어내는 것은 구동 중인 SIP
코어의 설정을 건드리는 일이라 얻는 것에 비해 위험이 큽니다.

#### 10-4 — 안드로이드 앱: 지금은 옮길 이유가 없습니다

계획서를 쓸 때의 질문은 *"Janus 로 옮길지, SIP + rtpengine 으로 갈지"* 였습니다.
7단계까지 와서 보니 **질문의 전제가 달라졌습니다.**

**Janus 는 브라우저를 위한 게이트웨이입니다.** 안드로이드 앱은 이미 SIP 단말이고,
Janus 반대편에 서는 쪽입니다. "앱을 Janus 로 옮긴다" 는 것은 앱을 WebRTC 클라이언트로
다시 만든다는 뜻인데, 지금 그럴 이유가 약합니다.

| | LAN | 셀룰러(외부) |
|---|---|---|
| 지금 앱 (pjsip, 평문 SIP/UDP) | ✅ 등록·통화·음성 다 됨 | ❌ 경로 없음 |
| 필요한 것 | 없음 — rtpproxy 가 중계 중 | SIP over TLS 또는 WSS + 미디어 릴레이 |

**rtpengine 은 아직 필요 없습니다.** 계획서 초기의 "rtpengine 이 있어야 한다" 는
전제는 LAN 구간에서는 틀렸습니다 — rtpproxy 가 이미 그 일을 하고 있고, 6단계에서
WebRTC ↔ 평문 G.711 브리징까지 확인됐습니다.

그래서 실제 갈림길은 **"셀룰러에서 붙게 할 것인가"** 이고, 그때 선택지는 셋입니다.

| 안 | 무엇이 필요한가 | 비고 |
|---|---|---|
| ① 지금 그대로 (LAN 전용) | 없음 | 앱이 집 밖에서는 안 됨 |
| ② SIP over TLS | 이미 `5061` 에 nginx stream 이 있음 | 미디어는 여전히 평문 RTP — 포워딩 필요 |
| ③ WSS + WebRTC | `/sip/` (있음) + **rtpengine** (없음) | 앱을 WebRTC 로 바꿔야 함 |

②는 시그널링만 감싸는 것이라 앱 변경이 작고, ③은 브라우저와 같은 길을 쓰지만
앱과 서버 양쪽에 일이 많습니다. **지금 진행 중이신 TLS 작업(`kamailio-tls.cfg`,
`nginx-sip-tls.conf`)이 ②의 절반**이므로, 그쪽을 먼저 마무리해 보고 그래도
부족할 때 ③을 꺼내는 순서를 권합니다.

> ⚠️ ②로 갈 때 **`5061` 포트가 겹칩니다.** 지금 그 주소는 nginx stream 이 잡고
> 있는데, 저장소의 `kamailio-tls.cfg` 도 `listen=tls:...:5061` 을 선언합니다.
> 둘 중 하나를 고르거나 포트를 갈라야 합니다 (앞서 확인한 내용).

#### ✅ 정해졌습니다 (2026-08-21) — 모바일도 **Janus** 로 붙습니다

TLS 를 걷어내면서(아래) 갈림길이 좁혀졌고, 모바일이 WebRTC 로 **어디에** 붙는지도
정해졌습니다.

| | 붙는 곳 | 결정 |
|---|---|---|
| ① **Janus** | `/janus-api` — 브라우저와 같은 길 | ✅ **이것** |
| ② Kamailio WS | `/sip/` — SIP over WebSocket | ❌ |

브라우저·모바일이 모두 같은 길을 쓰게 됩니다. 5~7단계에서 검증된 것이 정확히
그 길이므로, 앱 쪽이 늘리는 것은 "Janus 에 붙는 클라이언트" 하나뿐입니다.

**따라오는 결과 셋:**

1. **rtpengine 이 영구히 필요 없어집니다.** 그것을 요구하던 것은 ②뿐이었고,
   `websocket-plan.md` 가 2단계에서 막혀 있던 이유도 그것이었습니다. 이제 그
   막힘은 풀 필요가 없는 막힘이 됐습니다.
2. **SIP over TLS 도 필요 없습니다** — 걷어냈습니다
   (`../../kamailio/docs/mobile-transport.md` 7절).
3. **`/sip/`(Kamailio WS) 라우트에 쓸 사람이 없어집니다** — 10-3 을 다시 봐야
   합니다 (아래).

이제 SIP 는 **서버 안쪽의 신호 언어**로만 남습니다. 밖에서 SIP 로 들어오는
길은 두지 않습니다.
