...
| 10-3 | `/sip/`(Kamailio WS) 라우트를 유지할지 | 🔸 **재검토됨** — 끄기를 권함 (아래) |
...
**권하는 것 — 지우지 말고 `enabled = false` 로 내려 둡니다.**
...
지금은 라우트가 살아 있고(`/sip/` → 400, WS 업그레이드가 아니면 정상) 연결은
0 입니다. **대시보드가 이 서비스를 "정상" 으로 표시한다고 해서 누군가 쓰고 있다는
뜻은 아닙니다.**
